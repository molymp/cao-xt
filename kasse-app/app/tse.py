"""
CAO-XT Kassen-App – Fiskaly Cloud TSE Client
Implementiert KassenSichV / §146a AO via Fiskaly REST API v2.

Ablauf pro Kassiervorgang:
...
def _terminal_config(terminal_nr: int) -> dict:
    """Liest Fiskaly-Konfiguration aus XT_KASSE_TERMINALS."""
    with get_db() as cur:
        cur.execute(
    ...
    if not row['FISKALY_API_KEY']:
        raise RuntimeError(
            f"Terminal {terminal_nr}: Fiskaly API-Key nicht konfiguriert. "
            "Bitte in Admin → Terminal eintragen."
        )
    return row
...
def tse_verfuegbar(terminal_nr: int) -> bool:
    """Gibt True zurück wenn TSE erreichbar und konfiguriert."""
    try:
        tconf = _terminal_config(terminal_nr)
...
def tse_tss_status(terminal_nr: int) -> dict:
    ...
    Gibt das TSS-Objekt zurück, mindestens {'state': '...'}.
    """
    tconf = _terminal_config(terminal_nr)
    token = _get_token(terminal_nr, tconf)
    env   = tconf['FISKALY_ENV']
...
def _tss_put(url: str, body: dict, token: str,
    ...
    if not resp.ok:
        ...
        raise RuntimeError(
            f"TSS-Statuswechsel '{body.get('state')}' fehlgeschlagen "
