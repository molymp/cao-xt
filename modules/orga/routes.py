"""
CAO-XT Orga-Modul – Flask-Blueprint (Phase 1: Artikelpreispflege & VK-Ermittlung)

Registrierung:
    app.register_blueprint(bp, url_prefix='/orga')
...
def api_artikel_suche():
    """
    GET /orga/api/artikel/suche?q=<suchbegriff>&limit=50
...
def api_artikel_detail(artnum: str):
    """GET /orga/api/artikel/<artnum> – Artikeldetails inkl. aktueller VK-Preise."""
...
def api_vk(artnum: str):
    """
    GET /orga/api/artikel/<artnum>/vk?ebene=5

    Gibt den aktuell gültigen VK-Preis zurück (Quelle: orga | aktion | cao).
    """
...
def api_preishistorie(artnum: str):
    """
    GET /orga/api/artikel/<artnum>/preise
...
def api_preis_setzen(artnum: str):
    """
    POST /orga/api/artikel/<artnum>/preise
...
def api_preishistorie_alle():
    """
    GET /orga/api/preishistorie?artnum=<optional>&limit=200&offset=0
...
def api_warengruppen():
    """GET /orga/api/warengruppen – Alle Warengruppen für Filter-Dropdown."""
    return jsonify(m.warengruppen_liste())


@bp.get('/api/warengruppen/mit-faktor')
def api_warengruppen_mit_faktor():
    """GET /orga/api/warengruppen/mit-faktor – Warengruppen mit Durchschnitts-Faktor."""
...
def api_artikel_vk5_setzen(artnr: str):
    """
    PATCH /orga/api/artikel/<artnr>/vk5
...
def api_artikel_ek_setzen(artnr: str):
    """
    PATCH /orga/api/artikel/<artnr>/ek
...
def api_artikel_vpe_setzen(artnr: str):
    """
    PATCH /orga/api/artikel/<artnr>/vpe
