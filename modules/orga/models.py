"""
CAO-XT Orga-Modul – Datenbankmodelle & Geschäftslogik (Phase 1)
...
def _db_params() -> dict:
    ...
    analog zum Lazy-Pool-Pattern in orga-app/app/db.py.
    """
    import sys
    try:
...
def aktuellen_preis_holen(artnum: str, preisebene: int) -> dict | None:
    """
    Aktuell gültigen Orga-Preis für Artikel + Preisebene.
...
def vk_berechnen(artnum: str, preisebene: int) -> dict:
    """
    VK-Preis ermitteln: Priorität Orga-Preishistorie > CAO ARTIKEL_PREIS (Aktionspreis) > CAO VKxB.

    Rückgabe:
        {
            'preis_ct': <int>,      # Bruttopreis in Cent
            'quelle': 'orga' | 'aktion' | 'cao',
            'mwst_code': <str>,
            'artnum': <str>,
        }
    """
    ...
    # 1. Orga-Preishistorie
    orga_preis = aktuellen_preis_holen(artnum, preisebene)
    if orga_preis:
        return {
            'preis_ct': orga_preis['PREIS_BRUTTO_CT'],
            'quelle': 'orga',
