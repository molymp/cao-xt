...
# ── Buchen (Phase B): Wareneingang abschliessen ────────────────────
#
# Wichtig: Wareneingang = Lieferschein. Hier werden NUR die gelieferten
# Mengen bestaetigt. Preise gehoeren zur EK-Rechnung (siehe Phase C —
# 'Einkauf buchen', noch nicht umgesetzt). EKEINGANG_POS hat ohnehin
# keine Preisspalte.
#
# Beim Buchen erforderlich:
#   - LIEFNUM    (Lieferscheinnummer vom Lieferanten)
#   - LIEFDATUM  (Datum auf dem Lieferschein)
#
# Wirkung des Buchens:
#   - EKEINGANG.STADIUM 0 → 2 (unberechnet — wartet auf EK-Rechnung)
#   - alle EKEINGANG_POS.GEBUCHT_FLAG = 'Y'
#   - EKEINGANG.LIEFNUM / LIEFDATUM gespeichert
#
# Storno geht nur solange STADIUM=0 (vor dem Buchen). Nach dem Buchen
# muss in CAO-Faktura storniert werden.


def buchen(eingang_id: int, ma_id: int | None, ma_name: str | None, *,
           liefnum: str | None = None,
           liefdatum: 'date | str | None' = None) -> dict[str, Any]:
    """Bucht den Wareneingang (= Lieferschein abschliessen).

    Args:
        eingang_id: EKEINGANG.REC_ID
        ma_id: aktuelle Mitarbeiter-ID (fuer Audit, nicht verpflichtend)
        ma_name: Mitarbeiter-Anzeigename
        liefnum: Lieferscheinnummer vom Lieferanten — verpflichtend
        liefdatum: Lieferscheindatum (date oder ISO-String) — verpflichtend

    Wirft ``ValueError`` wenn LIEFNUM oder LIEFDATUM fehlt, ``LookupError``
    wenn der Wareneingang nicht existiert, ``PermissionError`` wenn er
    schon gebucht / storniert ist.
    """
    eingang_id = int(eingang_id)
    ma_name_safe = (ma_name or 'CAO-XT')[:100]
    liefnum = (liefnum or '').strip()
    if not liefnum:
        raise ValueError('Lieferscheinnummer fehlt')
    if isinstance(liefdatum, str):
        liefdatum = (liefdatum or '').strip()
        if not liefdatum:
            raise ValueError('Lieferscheindatum fehlt')
        try:
            from datetime import date as _date
            liefdatum = _date.fromisoformat(liefdatum)
        except ValueError:
            raise ValueError(f'Lieferscheindatum ungueltig: {liefdatum!r}')
    if liefdatum is None:
        raise ValueError('Lieferscheindatum fehlt')
    liefnum = liefnum[:50]
    ...
    with _eingang_lock_tx(eingang_id) as cur:
        ...
        cur.execute(
            "SELECT COUNT(*) AS n FROM EKEINGANG_POS WHERE EKEINGANG_ID = %s",
            (eingang_id,),
        )
        ...
        cur.execute(
            """UPDATE EKEINGANG
                  SET STADIUM   = 2,
                      LIEFNUM   = %s,
                      LIEFDATUM = %s,
                      ERST_NAME = COALESCE(NULLIF(ERST_NAME,''), %s)
                WHERE REC_ID = %s""",
...
def _bestellpos_synchronisieren(cur, eingang_id: int,
    ...
    Wareneingaenge bilden und:
      - menge_geliefert >= bestellmenge → STADIUM=95 (voll geliefert WE),
        EKEINGANG='Y'
      - 0 < menge_geliefert < bestellmenge → STADIUM=93 (Teillieferung WE),
        EKEINGANG='Y'
      - sonst → unveraendert

    Status 9 (voll berechnet) bleibt der EK-Rechnung vorbehalten
    (Phase C — folgt). 8 (rest nicht lieferbar) und 127 (storniert)
    sind Sonderstati und bleiben unangetastet.

    EKBESTELL_INFO.LIEFERTERMIN wird NICHT ueberschrieben — das ist
    der vereinbarte Wunschtermin, nicht das tatsaechliche Lieferdatum.
    Letzteres holt sich das UI per JOIN aus EKEINGANG_POS+EKEINGANG.

    ``liefdatum`` ist hier nur historisch noch im Parameter — wird nicht
    mehr verwendet, bleibt fuer API-Kompatibilitaet.

    Returns: Set der EKBESTELL_IDs deren Pos angefasst wurden.
    """
    del liefdatum  # bewusst nicht mehr verwendet, siehe Docstring
    cur.execute(
    ...
    for bpr in bestell_pos_rows:
        ...
        # Sonderstati nicht ueberschreiben
        if cur_st in (8, 9, 127):
            continue
        ...
        if geliefert + 0.0001 >= soll and soll > 0:
            neuer_st = 95   # voll geliefert WE
        else:
            neuer_st = 93   # Teillieferung WE
        cur.execute(
...
def _bestell_header_aktualisieren(cur, bestell_id: int) -> None:
    ...
    Logik:
      - alle aktiven Pos in (95, 8) und mind. eine in 95 → 95
        (voll geliefert WE)
      - mind. eine Pos in (93, 95) → 93 (Teillieferung WE)
      - sonst → unveraendert

    Stati 9 (voll berechnet) und 127 (storniert) sind Sonderstati und
    werden nicht geaendert. Pos in 9 zaehlen weder zu 'voll geliefert'
    noch zu 'teilgeliefert' fuer den Header — das macht erst Phase C
    (EK-Rechnung) sauber.
    """
    cur.execute(
    ...
    if not h:
        return
    if int(h['STADIUM']) in (9, 127):
        return  # voll berechnet oder storniert — nicht zurueckdrehen
    ...
    voll_geliefert = verteilung.get(95, 0) + verteilung.get(8, 0)
    hat_teil_oder_voll = (verteilung.get(93, 0) + verteilung.get(95, 0)) > 0

    if aktive_pos > 0 and voll_geliefert == aktive_pos and verteilung.get(95, 0) > 0:
        neu = 95
    elif hat_teil_oder_voll:
        neu = 93
    else:
        return  # alles noch offen oder voll-berechnet — Header bleibt
...
def lieferschein_setzen(eingang_id: int, *,
                        liefnum: str | None,
                        liefdatum: 'date | str | None') -> dict[str, Any]:
    """Setzt LIEFNUM / LIEFDATUM auf einem offenen Wareneingang.

    Felder duerfen einzeln gesetzt werden — ``None`` bedeutet 'unveraendert',
    ...
    with _eingang_lock_db(eingang_id) as cur:
        ...
        sets: list[str] = []
        params: list[Any] = []
        if liefnum is not None:
            sets.append('LIEFNUM = %s')
            params.append((liefnum or '').strip()[:50])
        if liefdatum is not None:
            if isinstance(liefdatum, str):
                liefdatum = (liefdatum or '').strip()
                if liefdatum == '':
                    sets.append('LIEFDATUM = NULL')
                else:
                    try:
                        from datetime import date as _date
                        liefdatum = _date.fromisoformat(liefdatum)
                        ...
                        raise ValueError(f'Lieferscheindatum ungueltig: {liefdatum!r}')
                    sets.append('LIEFDATUM = %s')
                    params.append(liefdatum)
            else:
                sets.append('LIEFDATUM = %s')
                params.append(liefdatum)
        if not sets:
            return {'ok': True, 'unveraendert': True}
        params.append(eingang_id)
        cur.execute(
            f"UPDATE EKEINGANG SET {', '.join(sets)} WHERE REC_ID = %s",
            params,
        )
    return {'ok': True}
