"""
CAO-XT Orga-Personal – Flask-Blueprint (P1 Stammdaten).
...
def detail(pers_id: int):
    ...
    if not ma:
        ...
        return redirect(url_for('orga_personal.uebersicht'))
...
def urlaub_korrektur_anlegen(pers_id: int):
    ...
    try:
        ...
        flash(f'Fehler beim Speichern: {e}', 'error')
    return redirect(url_for('orga_personal.detail',
...
def stundenkorrektur_anlegen(pers_id: int):
    ...
    if not m.ma_by_id(pers_id):
        flash('Mitarbeiter nicht gefunden.', 'error')
        return redirect(url_for('orga_personal.arbeitszeitkonten'))
    try:
...
def _firma_name() -> str:
    ...
    (wird in orga-app/app/config.py gesetzt), fallback XT_EINSTELLUNGEN,
    sonst leer."""
    try:
...
def stempel_korrektur_anlegen(pers_id: int):
    ...
    if not zeitpunkt:
        ...
        return redirect(url_for('orga_personal.stempel_ma', pers_id=pers_id))
    try:
...
def schicht_neu():
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for('orga_personal.schicht_uebersicht'))
        except (ValueError, LookupError) as e:
            flash(str(e), 'error')
        except Exception as e:
            flash(f'Fehler beim Speichern: {e}', 'error')
...
def _redirect_schichtplan(montag_raw: str, rueck: str):
    ...
    ziel = ('orga_personal.schichtplan_raster' if rueck == 'raster'
            else 'orga_personal.schichtplan')
...
def schichtplan_kopieren():
    ...
    try:
        ...
        return redirect(url_for('orga_personal.schichtplan',
                                woche=ziel_montag.isoformat()))
    except ValueError as e:
        flash(f'Fehler: {e}', 'error')
    except Exception as e:
        flash(f'Fehler beim Kopieren: {e}', 'error')
    return redirect(url_for('orga_personal.schichtplan', woche=quelle_raw))
