"""
CAO-XT Orga-Personal – Datenzugriff (DictCursor, kein ORM).
...
@contextmanager
def get_db_ro():
    """Read-only Cursor (alias fuer Orga-get_db, autocommit=True)."""
    with _get_db() as cur:
        yield cur
...
def status_fuer_dashboard(heute: date, preview_n: int = 3) -> dict:
    """Aggregat fuer das Orga-Hauptdashboard: Abwesenheiten heute + offene
    Urlaubsantraege.
...
# ── Benachrichtigungs-Hook (E-Mail-Verteiler aus Admin) ──────────────────

def benachrichtigung_empfaenger(bereich: str) -> list[str]:
    """Liefert aktive Empfaenger-Mailadressen fuer einen Bereich.

    Die Liste wird in der Admin-App unter /benachrichtigungen gepflegt.
...
def benachrichtigung_antrag_senden(*, bereich: str,
    ...
    text = (
        ...
        f'{details}\n\n'
        f'Bitte im Backoffice (Orga → Personal) pruefen und entscheiden.\n'
    )
    try:
