...
(siehe ``.gitignore``). Default-Pfad: ``modules/orga/personal/_seed_local/``.

Aufruf (aus Repo-Root)::

    # Bootstrap aus dem Default-Verzeichnis (erste gefundene .csv):
    python3 -m modules.orga.personal.tools.import_csv
    ...
    python3 -m modules.orga.personal.tools.import_csv --user 2
...
def _init_orga_pool() -> None:
    """Initialisiert den common.db-Pool mit Orga-Konfiguration.

    Wir emulieren, was orga-app/app/db.py beim Start der Flask-App macht –
    ohne die App selbst zu booten."""
    repo_root = os.path.normpath(
        os.path.join(os.path.dirname(__file__), '..', '..', '..', '..')
    )
    sys.path.insert(0, repo_root)
    sys.path.insert(0, os.path.join(repo_root, 'orga-app', 'app'))

    import config  # type: ignore
    from common.db import init_pool

    init_pool('orga_import_pool', db_config={
        'host':     config.DB_HOST,
        'port':     config.DB_PORT,
        'name':     config.DB_NAME,
        'user':     config.DB_USER,
