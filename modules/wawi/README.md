...
- **Python Flask** – Blueprint (`modules/wawi/routes.py`)
...
mysql -u <user> -p <db_name> < modules/wawi/schema.sql
```
...
|---------|-------|----------|
| GET | `/wawi/api/artikel/suche?q=` | Artikelsuche (CAO read-only) |
| GET | `/wawi/api/artikel/<artnum>` | Artikeldetails |
| GET | `/wawi/api/artikel/<artnum>/vk?ebene=5` | VK-Preis ermitteln |
| GET | `/wawi/api/artikel/<artnum>/preise` | Preishistorie |
| POST | `/wawi/api/artikel/<artnum>/preise` | Neuen Preis setzen (GoBD) |
| GET | `/wawi/api/preishistorie` | Preishistorie-Übersicht (Audit-Log) |
