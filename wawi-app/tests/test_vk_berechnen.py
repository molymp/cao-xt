"""
Tests für vk_berechnen – VK-Preis-Ermittlung im WaWi-Modul.
...
Getestet wird die Geschäftslogik der 3-stufigen Preisermittlung:
  1. WaWi-Preishistorie (XT_WAWI_PREISHISTORIE) hat höchste Priorität
...
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'modules', 'wawi'))
...
def _wawi_preis(artnum='A001', preis_ct=350, preisebene=5):
    """Minimaler WaWi-Preis-Dict wie von aktuellen_preis_holen() zurückgegeben."""
    return {
        'REC_ID': 1,
        'ARTNUM': artnum,
...
class TestVkBerechnenPrioritaeten(unittest.TestCase):
    ...
    def test_wawi_preis_hat_hoechste_prioritaet(
        self, mock_aktionen, mock_wawi, mock_artikel
    ):
        """WaWi-Preishistorie überschreibt Aktions- und CAO-Preis."""
        mock_artikel.return_value = _artikel(vk5b=2.99)
        mock_wawi.return_value = _wawi_preis(preis_ct=350)
        ...
        self.assertEqual(result['quelle'], 'wawi')
        self.assertEqual(result['artnum'], 'A001')

    @patch.object(m, 'artikel_by_artnum')
    @patch.object(m, 'aktuellen_preis_holen')
    @patch.object(m, 'preisgruppen_fuer_artikel')
    def test_aktionspreis_wenn_kein_wawi_preis(
        self, mock_aktionen, mock_wawi, mock_artikel
    ):
        """Aktionspreis wird verwendet, wenn kein WaWi-Preis vorhanden."""
        mock_artikel.return_value = _artikel(vk5b=2.99)
        mock_wawi.return_value = None  # kein WaWi-Preis
        mock_aktionen.return_value = [_aktionspreis(preis=2.49)]
    ...
    @patch.object(m, 'aktuellen_preis_holen')
    @patch.object(m, 'preisgruppen_fuer_artikel')
    def test_cao_fallback_wenn_kein_wawi_und_keine_aktion(
        self, mock_aktionen, mock_wawi, mock_artikel
    ):
        """CAO-Standardpreis (VKxB) ist letzter Fallback."""
        mock_artikel.return_value = _artikel(vk5b=2.99)
        mock_wawi.return_value = None
    ...
    def test_mwst_code_wird_weitergegeben(
        ...
        mock_wawi.return_value = _wawi_preis(preis_ct=100)
        mock_aktionen.return_value = []
    ...
    @patch.object(m, 'aktuellen_preis_holen')
    @patch.object(m, 'preisgruppen_fuer_artikel')
    def test_preisebene_wird_an_wawi_abfrage_weitergegeben(
        self, mock_aktionen, mock_wawi, mock_artikel
    ):
        ...
        mock_wawi.assert_called_once_with('A001', 3)
