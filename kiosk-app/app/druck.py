...
def _bon_bytes(warenkorb_id, positionen, gesamtbetrag_cent,
    ...
    for pos in positionen:
        ...
        if menge > 1:
            ...
            b.extend(f"{zeile2:<28} GP {gp:>8}\n".encode('cp437', errors='replace'))
        else:
            b.extend(f"{name[:24]:<24} {gp:>8}\n".encode('cp437', errors='replace'))
            b.extend(f"  EP {ep}\n".encode('cp437', errors='replace'))
...
def generiere_bon_text(
    ...
    for pos in positionen:
        ...
        if menge > 1:
            zeilen.append(name[:38])
            zeile2 = f"  {menge} x EP {ep}"
            zeilen.append(f"{zeile2:<28} GP {gp:>8}")
        else:
            zeilen.append(f"{name[:24]:<24} {gp:>8}")
...
def drucke_pickliste(
    ...
    gesamt_hinweis:      Betrag in Cent → zeigt 'Zahlung bei Abholung' + Betrag
                         (für Bestätigungsbon ohne Einzelpreise)
    """
    b = bytearray()
    ...
    for pos in positionen:
        ...
        if mit_preisen:
            ...
            b.extend(f"  {menge}x  {name[:22]}\n".encode('cp437', errors='replace'))
            raw(b'\x1b\x21\x00')
            if menge > 1:
                b.extend(f"      EP {ep:<10} GP {gp:>8}\n".encode('cp437', errors='replace'))
            else:
                b.extend(f"      {gp:>8}\n".encode('cp437', errors='replace'))
        else:
            raw(b'\x1b\x21\x10')      # doppelte Höhe
            b.extend(f"  {menge}x  {name[:26]}\n".encode('cp437', errors='replace'))
            raw(b'\x1b\x21\x00')
    ...
    # ── Betrag bei Abholung (Bestätigungsbon) ────────────────
    if gesamt_hinweis is not None:
        text("================================\n")
        raw(b'\x1b\x61\x01')          # zentriert
        raw(b'\x1b\x45\x01')          # fett
